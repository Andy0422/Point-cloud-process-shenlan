...
def octree_radius_search_fast(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    ...
    # case1 如果当前搜索点及搜索半径所构成的球面包含当前的网格，则选取当前网格中的所有数据点
    ...
    if root.is_leaf and len(root.point_indices) > 0:
        ...
        for i in range(diff.shape[0]):
            result_set.add_point(diff[i], root.point_indices[i])
        # 判断是否需要提前停止，提前停止的条件为：当前搜索点及搜索半径所构成的球面完全包含于当前网格中
    ...
    for c, child in enumerate(root.children):
        ...
        if False == overlaps(query, result_set.worstDist(), child):
            continue
        if octree_radius_search_fast(child, db, result_set, query): #递归搜索子节点
            return True
...
def main():
    ...
    db_size = 64000             # 数据点个数
    dim = 3                     # 数据点维度
    leaf_size = 4               # 每个叶子节点包含的最多数据点个数
    min_extent = 0.0001         # 继续划分的最小边长
    k = 8                       # 需要搜索的邻近点个数

    db_np = np.random.rand(db_size, dim)        # 构建数据集

    root = octree_construction(db_np, leaf_size, min_extent)        # 构建octree
